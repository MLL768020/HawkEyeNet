...
__all__ = ['SMPConv','SMPBlock', 'SMPCNN_ConvFFN']
...
class SMPConv(nn.Module):
    def __init__(self, planes, kernel_size, n_points, stride, padding, groups=1):
        super().__init__()
    ...
    def make_kernels(self):
        diff = self.weight_coord.unsqueeze(-2) - self.kernel_coord.reshape(1, 2, -1).transpose(1,
                                                                                               2)  # [1, n_points, kernel_size^2, 2]
...
class SMPCNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        self.small_conv = Conv(in_channels, out_channels, self.small_kernel, stride, self.small_kernel // 2, groups,
                               act=False)
