...
from .kan_convs.kan_conv import *
from .SMPConv import *
...
from ..modules.conv import Conv, DWConv, RepConv, GhostConv, autopad, LightConv,ApexDCN,HawkEyeConv,HawkEyeConvmul,HEConv
from ..modules.block import get_activation, ConvNormLayer, BasicBlock, BottleNeck, RepC3, C3, C2f, Bottleneck, Upsample, \
    Downsample
...
from ..extra_modules.ops_dcnv3.modules.fapn import *
from ..extra_modules.DCNv4_op import DCNv4
...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C3_DCNv2', 'C2f_DCNv2', 'BasicBlock_DCNv2', 'BottleNeck_DCNv2', 'DCNv4',
           'C3_DCNv3', 'C2f_DCNv3', 'BasicBlock_DCNv3', 'BottleNeck_DCNv3','DCNvv4',
           ...
           'C3_Ortho', 'C2f_Ortho', 'ASFF_2_FAM', 'ASFF_3_FAM','HSPP','HEGradDynFPN',
           ...
           'C3_Faster_Rep_EMA', 'C2f_Faster_Rep_EMA','HawkASPP0','HawkGradDynFPN0','HawkGradDynFPNmul','HawkASPPmul',
           ...
           'RFCAConv','ContextGuideFusionModule',
           ...
           'BasicBlock_RFCAConv', 'BottleNeck_RFCAConv','HawkEyeConvmul',
           ...
           'asf_attention_model','GradDynFPN','ASPP','VSSGradDynFPN', 'VSSASPP',
           ...
           'TopBasicLayer','noGradDynFPN','noASPP','HawkGradDynFPN','HawkASPP',
           ...
           'C2f_ContextGuided',
           'C2f_DAV','ASPPdcnv3','ASPPdcnv4','ASPPkan','ASPPSMPConv','v3GradDynFPN','v4GradDynFPN','KanGradDynFPN','SMPGradDynFPN',
           ]
...
class DCNV3_YOLO(nn.Module):
    def __init__(self, ouc, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
...
class ASFF_2_FAM(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[128, 256]):
        super(ASFF_2_FAM, self).__init__()
        ...
        self.conv = Conv(self.inter_dim, self.inter_dim, 3, 1, p=1)
        self.level = level
        ...
        # 添加FAM功能
        self.lateral_conv = FSM(inter_dim, inter_dim)
        self.offset = nn.Conv2d(channel[1], channel[0], 1, bias=False)
        self.conv_fam = nn.Conv2d(channel[0], channel[0], 1, bias=False)
        self.dcpack_l2 = DCNvv4(inter_dim,inter_dim)
    ...
    def forward(self, x):
        ...
        print('2:', input2.size())
        if self.level == 0:
            input = self.upsample(input2)
            print('input:', input.size())
            feat_arm = input1
            print('arm:', feat_arm.size())
            feat_s = self.conv_fam(input)
            print('s:', feat_s.size())
            offset = self.offset(torch.cat([feat_arm, feat_s], dim=1))
            print('set:', offset.size())
            feat_align = self.dcpack_l2(feat_s, offset)
            print('align:', feat_align.size())
            input2 = feat_align + feat_arm
            print('input2:', input2.size())
        elif self.level == 1:
            input1 = self.downsample(self.lateral_conv(input1))

        level_1_weight_v = self.weight_level_1(input1)
        level_2_weight_v = self.weight_level_2(input2)
        print('weights2:', level_2_weight_v.size())
...
class ASFF_3_FAM(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[128, 256, 512]):
        ...
        # 添加FAM功能
        self.lateral_conv = FSM(inter_dim, inter_dim)
        self.offset = nn.Conv2d(inter_dim * 2, inter_dim, 1, bias=False)
        self.conv_fam = nn.Conv2d(inter_dim // 2, inter_dim, 1, bias=False)
        self.dcpack_l2 = DCNv4(inter_dim, inter_dim, 3, 1, 1, 8)
    ...
    def forward(self, x):
        ...
        # 对齐操作
        feat_l = self.lateral_conv(input1 if self.level == 0 else (input2 if self.level == 1 else input3))
        feat_s1 = self.conv_fam(input2 if self.level == 0 else (input1 if self.level == 1 else input2))
        feat_s2 = self.conv_fam(input3 if self.level == 0 else (input3 if self.level == 1 else input1))
        offset1 = self.offset(torch.cat([feat_l, feat_s1 * 2], dim=1))
        offset2 = self.offset(torch.cat([feat_l, feat_s2 * 2], dim=1))
        feat_align1 = F.relu(self.dcpack_l2(feat_s1, offset1))
        feat_align2 = F.relu(self.dcpack_l2(feat_s2, offset2))
        input1 = feat_align1 + feat_l
        input2 = feat_align2 + feat_l
...
class GradDynFPN(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = ApexDCN(in_channels[0], in_channels[1], kernel_size=3, stride=2)

        # Upsample the third input
        self.upsample = CARAFE(in_channels[1], k_enc=3, k_up=5, c_mid=64, scale=scale)

        # 1x1 conv to align channels for concatenation
        self.conv1 = nn.Conv2d(in_channels[2], in_channels[1], kernel_size=1)

        self.cv1 = nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=False)
        self.act = nn.Sigmoid()
        ...
        self.ou = nn.Conv2d(in_channels[1] * 2, in_channels[1], kernel_size=1)

        # Final 1x1 conv to adjust channel size
        self.final_conv = nn.Conv2d(in_channels[1] * 2, out_channels, kernel_size=1)

    def forward(self, x, y=0, z=0):
        x1, x2, x3 = x


        # Downsample in1 to match in2 size if necessary
        if x1.size()[2:] != x2.size()[2:]:
            p1 = self.downsample(x1)
        else:
            p1 = x1

        # Align channels for x3 and upsample to match in2 size if necessary
        x3 = self.conv1(x3)
        ...
        # Concatenate in2 with p1 and p3
        ou1 = torch.cat([x2, p1], dim=1)
        ou2 = torch.cat([x2, p3], dim=1)

        ou1 = self.ou(ou1)
        ou2 = self.ou(ou2)

        # Apply attention mechanisms to in2
        c1 = self.act(self.cv1(torch.cat([torch.mean(x2, 1, keepdim=True), torch.max(x2, 1, keepdim=True)[0]], dim=1)))
        c2 = self.channel_att(x2)

        # Multiply ou1 with c1 and ou2 with c2
        f1 = torch.mul(ou1, c1)
        f2 = torch.mul(ou2, c2)
        ...
        # Final 1x1 conv to match the channel size of in2
        out = self.final_conv(out)

        return out

class VSSGradDynFPN(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = Conv(in_channels[0], in_channels[1], k=3, s=2)
        self.vss = VSSBlock(in_channels[1])
        # Upsample the third input
        self.upsample = CARAFE(in_channels[1], k_enc=3, k_up=5, c_mid=64, scale=scale)
    ...
    def forward(self, x, y=0, z=0):
        ...
        # Downsample in1 to match in2 size if necessary
        if x1.size()[2:] != x2.size()[2:]:
            p1 = self.vss(self.downsample(x1))
        else:
            p1 = x1
        ...
        return out
class HEGradDynFPN(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = HEConv(in_channels[0], in_channels[1], kernel_size=3, stride=2)
...
class v4GradDynFPN(nn.Module):
    ...
    def forward(self, x, y=0, z=0):
        ...
        return out
class HawkGradDynFPN(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = HawkEyeConv(in_channels[0], in_channels[1], kernel_size=3, stride=2)
...
class HawkGradDynFPN0(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = HawkEyeConv(in_channels[0], in_channels[1], kernel_size=3, stride=2, sampling=0)
...
class HawkGradDynFPNmul(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = HawkEyeConvmul(in_channels[0], in_channels[1], kernel_size=3, stride=2, sampling=0)
    ...
    def forward(self, x, y=0, z=0):
        ...
        return out
class KanGradDynFPN(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = KANConv2DLayer(in_channels[0], in_channels[1], kernel_size=3, stride=2,padding=1)
...
class SMPGradDynFPN(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2):
        super().__init__()

        # Only downsample the first input
        self.downsample = Conv(in_channels[0], in_channels[1], 3, 2)
...
class VSSASPP(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3
        self.cv1 = VSSBlock(c3)
        self.cv2 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv3 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv4 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv5 = Conv(4 * c3, c2, 1, 1)
...
class HawkASPP(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3
        self.cv1 = HawkEyeConv(c1, c3, 3, 1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return self.cv5(torch.cat((y, ll), 1))
class HawkASPPmul(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3
        self.cv1 = HawkEyeConvmul(c1, c3, 3, 1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return self.cv5(torch.cat((y, ll), 1))
class HawkASPP0(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3
        self.cv1 = HawkEyeConv(c1, c3, 3, 1,sampling=0)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return self.cv5(torch.cat((y, ll), 1))
class ASPPdcnv3(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.cv= Conv(c1, c3, k=1)
        self.cv1 = DCNV3_YOLO(c3, k=3, s=1)
        self.cv2 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv3 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv4 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv5 = Conv(4 * c3, c2, 1, 1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return self.cv5(torch.cat((y, ll), 1))
class ASPPdcnv4(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.cv= Conv(c1, c3, k=1)
        self.cv1 = DCNv4(c3, kernel_size=3, stride=1, pad=1, group=1, dilation=1)
        self.cv2 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv3 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv4 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv5 = Conv(4 * c3, c2, 1, 1)
...
class ASPPkan(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3
        self.cv1 = KANConv2DLayer(c1, c3, kernel_size=3,padding=1)
        self.cv2 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv3 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv4 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv5 = Conv(4 * c3, c2, 1, 1)
...
class ASPPSMPConv(nn.Module):
    # spp-elan
    def __init__(self, c1, c2, c3=256):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = c3
        self.cv0 = Conv(c1,c3,3,1)
        self.cv1 = SMPConv(c3,  kernel_size=3, stride=1,n_points=4,groups=1,padding=1)
        self.cv2 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv3 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv4 = nn.MaxPool2d(kernel_size=5, stride=1, padding=5 // 2)
        self.cv5 = Conv(4 * c3, c2, 1, 1)
